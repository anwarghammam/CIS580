...
def get_cpu_per_container():
    ...
    for node in Instance.nodes:
        ...
        r = requests.get('http://192.168.99.111:9090/api/v1/query?query=sum(irate(container_cpu_usage_seconds_total%7Bcontainer_label_com_docker_swarm_node_id%3D~"'+str(node.cluster_id)+'"%2C%20id%3D~"%2Fdocker%2F.*"%7D%5B5m%5D))%20by%20(name)%20*%20100%20&g0.tab=1')
...
def get_mem_per_container():
    ...
    for node in Instance.nodes:
        ...
        r = requests.get('http://192.168.99.111:9090/api/v1/query?query=avg_over_time(container_memory_usage_bytes%7Bcontainer_label_com_docker_swarm_node_id%3D~"'+str(node.cluster_id)+'"%2C%20id%3D~"%2Fdocker%2F.*"%7D%5B5m%5D)%2F1024%2F1024&g0.tab=1')
...
def get_Maxmem_penode():
    ...
    for node in Instance.nodes:
        ...
        r = requests.get('http://192.168.99.111:9090/api/v1/query?query=sum(node_memory_MemTotal_bytes%20*%20on(instance)%20group_left(node_name)%20node_meta%7Bnode_id%3D~"'+str(node.cluster_id)+'"%7D)%2F1000%2F1000&g0.tab=1')
